...
export default function BestProduct() {
    ...
    useEffect(() => {
        async function fetchList() {
            try {
                const res = await axios.get('https://panda-market-api.vercel.app/products?page=1&pageSize=10&orderBy=favorite')
        ...
        <>
            {list.map((item) => (
                <div key={item.id} className="flex flex-col cursor-pointer">
                    <img src={item.images} className="rounded-lg w-[221px] h-[221px] object-cover"/>
